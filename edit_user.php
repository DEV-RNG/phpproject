...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"Le mot de passe utilisateur a été mis à jour ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d',' Désolé, échec de la mise à jour du mot de passe!');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Update Le compte a <?php echo remove_junk(ucwords($e_user['name'])); ?>
        </strong>
       </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          Changer le mot de passe a <?php echo remove_junk(ucwords($e_user['name'])); ?>
        </strong>
      </div>
      <div class="panel-body">
