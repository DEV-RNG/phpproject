...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Désolé!</b> Nom de groupe déjà dans la base de données!');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Désolé!</b> Niveau de groupe déjà dans la base de données!');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          ...
          //failed
          $session->msg('d',' Désolé, impossible de créer le groupe!');
          redirect('add_group.php', false);
        }
   } else {
